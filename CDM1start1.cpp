...
std::vector<double> CDM1start1Rcpp(std::vector<double>& data, const std::string& permutation, double tol) {
    ...
    double x00ac = sum_range({0, 2, 8, 10});
    double x01ac = sum_range({1, 3, 9, 11});
    double x10ac = sum_range({4, 6, 12, 14});
    double x11ac = sum_range({5, 7, 13, 15});
    ...
    double g5 = x00ac - x11ac;
    double g6 = x00bc - x11bc;

    double g = std::max(-1 + tol, std::min((g1 + g2 + g3 + g4 + g5 + g6) / 6, 1 - tol));
    ...
    double xac = calc_x(x00ac, x11ac, x01ac, x10ac);
    double xbc = calc_x(x00bc, x11bc, x01bc, x10bc);

    auto calc_xa = [](double a, double b, double c) { return std::sqrt(std::min(a * b / c, 1.0)); };

    double x1a = calc_xa(xoc, xoa, xac);
    double x2a = calc_xa(xac, xob, xbc * xoa);
    double x3a = calc_xa(xac, xoa, xoc);
    double x4a = calc_xa(xbc, xoc, xob);
    double x5a = calc_xa(xbc, xob, xoc);
